...
Current release **v0.3.9**. Both HDZero and analog Betaflight targets are first-class (Phase 3, v0.3.0). v0.3.1 added an in-browser pixel editor for single glyphs and mini-logos. v0.3.2 was an audit-driven polish pass — race-condition fixes, persistent error banner for storage failures, modal focus trap + aria-labels, runtime shape validation on JSON / IndexedDB reads, inline errors throughout (no more `alert()` dialogs). v0.3.3 stabilized TTF palette layers (per-glyph random colors no longer reshuffle on background or tab changes), shipped a swatch-based palette editor with a `↻ reroll` button, and made the Edit TTF form live-preview every field while you dial it in. v0.3.4 added SVG support for glyph overrides and surfaces per-override decode errors inline. v0.3.5 added a scale knob to glyph overrides. v0.3.6 extended the same scale knob to logo layers (BTFL banner + mini-logo). v0.3.7 added a live preview on each Decoration logo card. v0.3.8 was an editor-overhaul batch: canvas scroll fix, unified logo editor (scale lives inside the modal alongside paint tools), BTFL banner now drawable, 1–16 px brush for pencil/eraser with hover outline, shift-drag axis-locked straight lines. v0.3.9 hotfixes a rules-of-hooks violation in FontPreview that caused stale canvas content + theme glitches when toggling rapidly between HDZero and Analog modes.
